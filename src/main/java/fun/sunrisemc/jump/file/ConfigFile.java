package fun.sunrisemc.jump.file;
...
public class ConfigFile {
    ...
    public static boolean save(@NonNull String filename, @NonNull YamlConfiguration config) {
        ...
        try {
            ...
            DoubleJumpPlugin.logWarning("Failed to save configuration for " + filename + ".yml.");
            e.printStackTrace();
            return false;
        }
    }
    ...
    public static boolean create(@NonNull String filename) {
        try {
            DoubleJumpPlugin.getInstance().saveResource(filename + ".yml", false);
        } catch (Exception e) {
            DoubleJumpPlugin.logWarning("Failed to save default configuration for " + filename + ".yml.");
            e.printStackTrace();
            return false;
        }
        return true;
    ...
    private static File getFolder() {
        File pluginFolder = DoubleJumpPlugin.getInstance().getDataFolder();
    ...
    private static YamlConfiguration getDefault(@NonNull String name) {
        ...
        try {
            InputStream resourceStream = DoubleJumpPlugin.getInstance().getResource(name + ".yml");
            if (resourceStream != null) {
                InputStreamReader reader = new InputStreamReader(resourceStream, StandardCharsets.UTF_8);
                defaultConfig.load(reader);
            }
        } catch (Exception e) {
            DoubleJumpPlugin.logWarning("Failed to load default configuration for " + name + ".yml.");
            e.printStackTrace();
        }
